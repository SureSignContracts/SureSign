...
//
// Phase C fixed the wizard's real, pre-existing bug: every newly-uploaded
// Contract was silently saved as `type: 'main_contract'` regardless of what
// the user actually uploaded (a Subcontractor's subcontract, a Consultant's
// appointment, a Supplier's agreement) — wrong for anyone who isn't a Main
// Contractor. The upload step now asks for Contract Type explicitly before
...
function AiContractWizard({ projectId, aiAnalyses, onClose, onCreated }: {
  ...
  // Phase C — must be chosen before the stub Contract is created (see
  // handleAnalyse()/createAndAnalyseMutation below). Deliberately starts
  // empty — no silent main_contract default — since a Subcontractor,
  // Consultant, or Supplier uploading their own agreement is just as valid
  // as a Main Contractor uploading a main contract, and the AI response
  // never reliably determines this (see mapExtractedToForm — it never
  // sets `type` at all, by design).
  ...
  const [form, setForm] = useState<ContractForm>({
    // Phase C — no silent main_contract default; see uploadContractType's
    // own comment above for why. The 'form' step's own Contract Type field
    // (already required) forces an explicit, real choice for the
    // existing-analysis path; the new-upload path carries the value chosen
    // in handleAnalyse() forward instead of leaving this empty.
    title: '', type: '', reference_number: '', form_of_contract: '',
  ...
  const createAndAnalyseMutation = useMutation({
    // Phase C — `type` is now the user's own explicit choice from the
    // upload step (uploadContractType), passed in alongside the file
    // rather than closed over, so there's no ambiguity about which value a
    // given call used. Previously hardcoded to 'main_contract' regardless
    // of what the uploaded document actually was — see this component's
    // own top-level comment and the Phase C final report for the full bug.
  ...
  // Poll for analysis completion — Phase C shared hook (previously an
  // inline setInterval identical to ContractAnalysisReview's own, now
  ...
  function handleAnalyse() {
    if (!contractFile || !uploadContractType) return;
    // Carry the user's real choice into `form.type` now — mapExtractedToForm()
    // never sets `type` (the AI response doesn't reliably determine it), so
    // without this the 'form' step would otherwise show an empty Contract
    // Type despite the stub Contract already having been saved with the
    // correct one above.
    setForm(prev => ({ ...prev, type: uploadContractType }));
    setStep('analysing');
    createAndAnalyseMutation.mutate({ file: contractFile, type: uploadContractType });
  }
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {step === 'upload' && (
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-xs mb-1" style={{ color: 'var(--text-muted)' }}>Contract Type *</label>
                <Select
                ...
                <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                  What kind of agreement is this? This isn&rsquo;t guessed from the document — choose the one that matches what you&rsquo;re uploading.
                </p>
              </div>
              <input
                ref={fileInputRef}
          ...
          {step === 'form' && (
            <form
              ...
              onSubmit={e => {
                e.preventDefault();
                // The submit button's own `disabled` (below) only guards a
                // click — pressing Enter inside any text input in this form
                // still fires this handler directly. The backend already
                // rejects an empty type with a 422 either way, but this
                // avoids surfacing that as a confusing generic error toast.
                if (!form.type) return;
        ...
        <div className="flex items-center justify-between p-5 flex-shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          {step === 'upload' && (
            <button
              type="button"
              disabled={!contractFile || !uploadContractType}
              onClick={handleAnalyse}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium"
              style={{
                backgroundColor: contractFile && uploadContractType ? 'var(--gold)' : 'var(--bg-elevated)',
                color: contractFile && uploadContractType ? 'var(--accent-fg)' : 'var(--text-muted)',
              }}
            >
              <Sparkles size={14} />
              Analyse Contract
          ...
          {step === 'form' && (
            <button
              type="submit"
              form="wizard-form"
              // App-level validation, not just the field's native `required`
              // (the Contract Type Select doesn't actually forward one — see
              // InputField's options branch — matching this repo's Dropdown
